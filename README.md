...
Для fashion‑товаров AI может создать сценарий модельной примерки.

Следующие модули показываются как **beta** и пока не входят в текущую автоматическую выдачу финальных изображений:

- чистый product packshot;
- lifestyle‑сцена;
- модельная примерка на AI‑модели.

Текущий пакет создаёт для них конкретный визуальный бриф. Интерфейс не обещает точность размера, посадки или готовую фотографию.
